...
myBurrowFeatures <- function(myBurrowData) {
  ...
  names.1 <- names(myFeatures)
  names.2 <- !grepl("^FILE_NAME$|^COLUMN_NAME$|^BEST_GUESS$|^GUESS_SOURCE$",names.1)
  names.3 <- c("FILE_NAME","COLUMN_NAME","BEST_GUESS","GUESS_SOURCE")
  names.4 <- names.1[names.2]
  names.5 <- c(names.3,names.4)
  myFeatures <- subset(myFeatures,,names.5)
