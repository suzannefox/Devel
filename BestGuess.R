...
# get features of a variable where there is data
myVariable.Features <- subset(Burrow, InfoType=="FEATURE" & Variable1=="Species" & myData1 > 0)

myVariable <- "Species"
test.bestguess <- "DK"
test.guesssource <- "DK"
test.choose <- subset(Burrow, Variable1==myVariable & myData1 > 0)
...
for (i in 1:length(regex_precedence)) { 
  ...
  if (diagnostics==TRUE) print(paste(myVariable,"testing for",regex_precedence[i]))
  
  test.if <- subset(test.choose, InfoDetail==regex_precedence[i] & myData1 > 0.95)
  if (nrow(test.if) > 0) {
    #latitude and longitude are special cases
    if (regex_precedence[i]=="REGEX_GEO_LATITUDE" & myVariable=="LATITUDE") {
      test.bestguess <- gsub("REGEX_","",regex_precedence[i])
      test.guesssource <- "REGEX MATCH"
      
    } else if (regex_precedence[i]=="REGEX_GEO_LONGITUDE" & myVariable=="LONGITUDE") {
      test.bestguess <- gsub("REGEX_","",regex_precedence[i])
      test.guesssource <- "REGEX MATCH"
    }
    
    if (regex_precedence[i]=="REGEX_GEO_LATITUDE" | regex_precedence[i]=="REGEX_GEO_LONGITUDE") {
      next
    }
    ...
    test.bestguess <- gsub("REGEX_","",regex_precedence[i])
    test.guesssource <- "REGEX MATCH"
    if (diagnostics==TRUE) print(paste("BEST GUESS IS",test.bestguess))
...
if (test.bestguess=="DK") {
  if (diagnostics==TRUE) print(paste("........................... Decimals test :", myVariable))
...
# test for category
if (test.bestguess=="DK") {
  if (diagnostics==TRUE) print(paste("........................... Category test :", myVariable))
  ...
  if (nrow(test.if) == 3) {
    ...
    if (diagnostics==TRUE) print(test.if)
    if (diagnostics==TRUE) print(test.alphas)
    if (diagnostics==TRUE) print(test.uniques)
    if (diagnostics==TRUE) print(test.uniquesden)
...
if (test.bestguess=="DK") {
  ...
  if (nrow(test.if) == 2) {
    ...
    if (diagnostics==TRUE) print(test.if)
    if (diagnostics==TRUE) print(test.digits)
    if (diagnostics==TRUE) print(test.uniquesden)
...
if (diagnostics==TRUE) {
  print(paste("... BestGuess ... for",test.Variable,"is",test.bestguess,"becasue",test.guesssource))
}
